...
import gzip
import pickle
import network_generator as graph_generator 
from network_generator import phy_network_generator 
from network_generator import sfc_network_generator 

def Main():
    PHY = graph_generator.phy_network_generator.FromGml(path="atlanta.gml", node_cap=[10,100,1],link_cap=[10,100,1])
    nx.draw_networkx(PHY)
    SFC_LIST = graph_generator.sfc_network_generator.PfoSFCSET(sfc_count=2, node_count_params=[5,6,5], node_req_params=[10,100,1], link_req_params=[10,100,1], flex_rate_params=0)
