...
def PfoSFC(node_count_params: list, node_req_params: list, link_req_params: list, flex_rate_params: float = -1):
    node_count = random.randrange(start=node_count_params[0], stop=node_count_params[1], step=node_count_params[2])
    ...
    for i in range(node_count):
        node_req = random.randrange(start=node_req_params[0], stop=node_req_params[1], step=node_req_params[2])
        link_req = random.randrange(start=link_req_params[0], stop=link_req_params[1], step=link_req_params[2])
    ...
    for i in range(node_count - 1):
        links.append((nodes[i][0], nodes[i+1][0],{"weight": flex_links[nodes[i][0]]}))
